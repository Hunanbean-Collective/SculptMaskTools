...
class MaskToolsPanel(Panel):
	...
	bpy.types.Screen.open_vgroup = bpy.props.BoolProperty(default=False, description = "Save to or Modify Mask Vertex Group")
	bpy.types.Screen.open_mask = bpy.props.BoolProperty(default=False, description = "Import Mask from Vertex")
	bpy.types.Screen.open_smoothsharp = bpy.props.BoolProperty(default=False, description = "Smooth/Sharp")
	bpy.types.Screen.open_fatless = bpy.props.BoolProperty(default=False, description = "Expand or Contract Mask")
	bpy.types.Screen.open_edgecavity = bpy.props.BoolProperty(default=False, description = "Assign Edge or Cavity as Mask")
	...
	def draw(self, context):
		...
		row.prop(context.screen, "open_vgroup",
			...
			text="Save / Modify Mask Vertex Group :", emboss=True)
		...
		if context.screen.open_vgroup:
			...
			col.scale_y = 1.3
			col.operator("mesh.masktovgroup", text = "Save Mask to New Vertex Group", icon = 'GROUP_VERTEX')
			row = box.row(align = True)
			row.operator("mesh.masktovgroup_append", text = "+Mask to VG", icon = 'EXPORT')
			row.operator("mesh.masktovgroup_remove", text = "-Mask from VG", icon = 'UNLINKED')
			# row.scale_x = 0.5
		...
		row.prop(context.screen, "open_mask",
			...
			text="Load / Modify Mask from Vertex Group:", emboss=True)
		...
		if context.screen.open_mask:
			...
			row = box.row(align=True)
			row.scale_y = 1.3
			row.operator("paint.clear_all_masks", text="Clear Visible Mask", icon='X')

			row = box.row(align = True)
			row.scale_y = 1.3
			row.operator("mesh.vgrouptomask_append", text = "+VG to Mask", icon = 'IMPORT')
			row.operator("mesh.vgrouptomask_remove", text = "-VG from Mask", icon = 'UNLINKED')
			row = box.row()
			row.operator("mesh.vgrouptomask", text = "Replace Visible Mask with VG Mask", icon='NONE')
			space = box.row()
		...
		row.prop(context.screen, "open_smoothsharp",
			...
			text="Mask Smooth or Sharpen :", emboss=True)
		...
		row.prop(context.screen, "open_fatless",
			...
			text="Mask Expand or Contract :", emboss=True)
		...
		row.prop(context.screen, "open_edgecavity",
			...
			text="Mask from Edges or Cavity :", emboss=True)
		...
		if context.screen.open_misc:
			...
			space = box.row()
			row = box.row(align = True)
			row.operator("masktools.mask_extrude", text = "Extrude",icon="ORIENTATION_NORMAL")
			row = box.row(align = True)
			row.prop(bpy.context.scene,"mask_extrude_volume", text = "")
			row.prop(bpy.context.scene,"mask_extrude_edgerelax", text = "", icon = "MOD_CURVE")
			col = box.column(align=True)
			...
			split = box.split()
			col = split.column(align=True)
			col.label(text="Uses Looptools Addon.")
