...
class MaskExtrude(bpy.types.Operator):
	''' Mask Extrude '''
	bl_idname = "masktools.mask_extrude"
	bl_label = "Mask Extrude"
	bl_options = {'REGISTER', 'UNDO'}
	...
	# bpy.types.Scene.mask_extrude = bpy.props.FloatProperty(name = "Mask Extrude Volume", default = -0.2)
	bpy.types.Scene.mask_extrude_volume = bpy.props.FloatProperty(name = "Extrude Volume", default = 0.2, min=-100, max=100)
	bpy.types.Scene.mask_extrude_edgerelax = bpy.props.BoolProperty(name = "Extrude Edge Relax", default = True)

	def execute(self, context):
		mask_extrude_volume = context.scene.mask_extrude_volume # update property from user input
		mask_extrude_edgerelax = context.scene.mask_extrude_edgerelax # update property from user input
		...
		if context.active_object.mode == 'SCULPT' :
			...
			bpy.ops.mesh.reveal()
			if mask_extrude_edgerelax == True:
				bpy.ops.mesh.region_to_loop()
				bpy.ops.mesh.looptools_relax(input='selected', interpolation='cubic', iterations='25', regular=True)
				bpy.ops.mesh.loop_to_region()
			bpy.ops.mesh.extrude_region_shrink_fatten(MESH_OT_extrude_region={},TRANSFORM_OT_shrink_fatten={"value": mask_extrude_volume,})
			bpy.ops.sculpt.sculptmode_toggle()
...
class MaskOutlinerelax(bpy.types.Operator):
	...
	def poll(cls, context):
		...
		return context.active_object is not None and context.active_object.mode == 'SCULPT'
	bpy.types.Scene.mask_outlinerelax_remove_doubles = bpy.props.FloatProperty(name = "Extrude Volume", default = 0.00, min=0, max=10)
...
def register():
	...
	bpy.types.Scene.mask_extrude_volume = MaskModSmooth.mask_extrude_volume
	bpy.types.Scene.mask_extrude_edgerelax = MaskModSmooth.mask_extrude_edgerelax
...
def unregister():
	...
	bpy.types.Scene.mask_extrude_volume
	bpy.types.Scene.mask_extrude_edgerelax
